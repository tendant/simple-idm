...
import type { Login, TwoFactorMethod, TwoFactorMethods } from '../api/login';
...
const LoginDetail: Component = () => {
  ...
  const toggleTwoFactorMethod = async (methodType: string, currentStatus: boolean) => {
    ...
    try {
      ...
      console.error(`Failed to ${currentStatus
        ? 'disable'
        : 'enable'} 2FA method:`, err);
      setError(err instanceof Error
        ? err.message
        : `Failed to ${currentStatus
          ? 'disable'
          : 'enable'} 2FA method`);
    }
    finally {
  ...
  return (
    ...
    >
      ...
      >
        <div
          class="mt-8 overflow-hidden bg-white shadow sm:rounded-lg"
        >
          <div
          ...
          >
            <dl>
              ...
              >
                ...
                >
                  <span
                    class={`inline-flex items-center rounded-md px-2.5 py-0.5 text-sm font-medium ${login()?.status === 'active'
              ...
              >
                ...
                <dd
                  class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0"
                >
                  {formatDate(login()?.last_modified_at)}
                </dd>
              </div>
              <div
                class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6"
              >
                ...
                <dd
                  class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0"
                >
                  {formatDate(login()?.password_last_changed)}
                </dd>
              </div>
            </dl>
          </div>
        ...
        >
          <h2
            class="text-xl font-semibold text-gray-11"
          >
          ...
          <Show
            when={!loadingTwoFactorMethods() && twoFactorMethodsResponse().count > 0 && twoFactorMethods().length > 0}
          >
            <div
              class="mt-4 overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg"
            >
              <table
                  ...
                  >
                    {method => (
                      <tr>
                        ...
                        >
                          <span
                            class={`inline-flex items-center rounded-md px-2.5 py-0.5 text-sm font-medium ${method.enabled
                              ? 'bg-green-100 text-green-800'
                              : 'bg-red-100 text-red-800'}`}
                          >
                          ...
                          >
                            <button
                              ...
                              class={`px-3 py-1 rounded-md text-sm font-medium ${method.enabled
                                ...
                                ${(processingMethod() === method.type || deletingMethod() === method.type)
          ...
          <Show
            when={!loadingTwoFactorMethods() && twoFactorMethodsResponse().count === 0}
          >
            <div
        ...
        >
          ...
          <Show
            when={!loadingUsers() && associatedUsers().length === 0}
          >
            <div
              ...
              >
                ...
                >
                  <h3
                    class="text-sm font-medium text-yellow-800"
                  >
                    No users are associated with this login
                  </h3>
                </div>
              </div>
            </div>
