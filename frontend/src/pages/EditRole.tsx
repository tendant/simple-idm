...
const fetchRole = async (uuid: string): Promise<Role> => {
  if (!uuid) {
    throw new Error('No role UUID provided');
  }
...
const EditRole: Component = () => {
  ...
  const [role] = createResource(() => params.uuid, fetchRole);
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <ErrorBoundary fallback={(err) => (
        ...
        <Suspense fallback={<div class="text-center">Loading...</div>}>
          <Show when={role()} fallback={<div class="text-center">No role data available</div>}>
            {(roleData) => (
              <>
                ...
                <div class="mt-8">
                  <RoleForm
                    ...
                    initialData={{ name: roleData.name || '' }}
                  />
                </div>
              </>
            )}
